...
export default function RefineFrame({
  ...
  useEffect(() => {
    const s = sim.current;
    // Parche sobre el original: sin poner raf en 0, el id cancelado queda
    // guardado y wake() cree para siempre que ya hay un frame pedido, así que
    // el bucle no vuelve a arrancar tras un remontaje (StrictMode lo provoca
    // en cada carga) y el lienzo se queda vacío.
    return () => {
